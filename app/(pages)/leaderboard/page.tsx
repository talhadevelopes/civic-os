        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {tabs.map((tab) => (
            <Link
              ...
              className={`px-4 py-2 text-[10px] font-bold uppercase tracking-widest rounded-xl border transition-all no-underline ${sortKey === tab.key
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            {top3.map((mla, i) => (
              <Link
                key={mla.slug}
                href={`/authorities/mla/${mla.slug}`}
                className={`relative overflow-hidden p-6 rounded-[2rem] shadow-xl group cursor-pointer no-underline hover:-translate-y-2 hover:scale-[1.02] transition-all ${i === 0 ? "bg-green-600 text-white" : "bg-white border border-gray-100 text-gray-900"
                  }`}
              >
