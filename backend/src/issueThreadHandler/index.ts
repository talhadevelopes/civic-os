...
export const issueThreadHandler = Router();

// ==================== COMMENTS ====================

// Get all comments for an issue
issueThreadHandler.get("/issues/:issueId/comments", async (req, res) => {
  ...
  try {
    ...
    const formattedComments = comments.map(comment => {
      ...
      if (comment.authorType === 'CITIZEN' && comment.citizenAuthor) {
        author = { ...comment.citizenAuthor, type: 'CITIZEN' };
      } else if (comment.authorType === 'MLA' && comment.mlaAuthor) {
        author = { ...comment.mlaAuthor, type: 'MLA' };
      } else if (comment.authorType === 'ORGANIZATION' && comment.orgAuthor) {
        author = { ...comment.orgAuthor, type: 'ORGANIZATION' };
      }
...
issueThreadHandler.post("/issues/:issueId/comments", async (req, res) => {
  const { issueId } = req.params;
  ...
  try {
    ...
    if (authorType === 'CITIZEN') {
      const citizenExists = await prisma.citizen.findUnique({
        where: { id: authorId }
      });
      if (!citizenExists) {
      ...
      const mlaExists = await prisma.mLA.findUnique({
        where: { id: authorId }
      });
      if (!mlaExists) {
...
issueThreadHandler.delete("/comments/:commentId", async (req, res) => {
...
issueThreadHandler.get("/issues/:issueId/upvotes", async (req, res) => {
  const { issueId } = req.params;
...
issueThreadHandler.post("/issues/:issueId/upvote", async (req, res) => {
  const { issueId } = req.params;
  ...
  try {
    ...
    if (existingUpvote) {
      ...
      return res.status(200).json({
        ...
        upvoted: false,
        upvoteCount: updatedIssue?.upvoteCount || 0
      });
    } else {
      // Add upvote
      await prisma.$transaction([
...
issueThreadHandler.get("/issues/:issueId/upvote-list", async (req, res) => {
  const { issueId } = req.params;
...
issueThreadHandler.get("/issues/:issueId/thread", async (req, res) => {
  const { issueId } = req.params;
  const { citizenId } = req.query;
